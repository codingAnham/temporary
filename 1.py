...
def run_multip(max_process, exec_func, q, tar_dir):
    p_list = []
    for i in range(max_process):
        p = multiprocessing.Process(target=exec_func, args=(q, tar_dir,))
        p_list.append(p)
        p.daemon = True
        p.start()
    return p_list
...
def make_tar_file(tar_name, tar_dir, org_files_list):
...
def upload_file(q, tar_dir):
    ...
    while True:
        ...
        if mp_data == quit_flag:
            break
        try:
            make_tar_file(tar_name, tar_dir, org_files_list)
...
def upload_file_multi(src_dir, tar_dir):
    success_log.info('%s directory is uploading' % src_dir)
    p_list = run_multip(max_process, upload_file, q, tar_dir)
...
if __name__ == '__main__':
    ...
    for x in range(len(src_dirs)):
        ...
        if cmd == 'upload_sbe':
            total_files = upload_file_multi(src_dirs[x], tar_dirs[x])
        # upload_log()
    """
    else:
        s3_booster_help
